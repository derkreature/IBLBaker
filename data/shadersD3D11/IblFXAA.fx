...
float4 rcpFrame = float4(1.0f / 1280, 1.0f / 1080, 0,0);
...
FxaaVS_Output FxaaVS(VS_INPUT input)
{
	FxaaVS_Output OUT;
	OUT.Pos =  float4(input.position.xyz, 1.0f);
	OUT.Tex.xy = input.tex0.xy;
	return OUT;
}
...
{
	FxaaTex tex = { anisotropicSampler, sourceTexture };
...
technique11 postprocess
{
	pass P0
	{
		SetVertexShader( CompileShader( vs_4_0, FxaaVS()));
