...
namespace
{
    ...
    void closestPointOnTriangle(float& ox, float &oy, float px, float py, float ax, float ay, float bx, float by, float cx, float cy)
    {
        ...
        const float m = Ibl::minValue(lab, Ibl::minValue(lbc, lca));
        if (m == lab)
        {
            ox = abx;
            oy = aby;
    ...
    Ibl::IVertexDeclaration* PosColorVertex::ms_decl;
    ...
    Ibl::IVertexDeclaration* PosColorUvVertex::ms_decl;
    ...
    Ibl::IVertexDeclaration* PosUvVertex::ms_decl;
    ...
    Ibl::IVertexDeclaration* PosNormalVertex::ms_decl;
...
struct Imgui
{
    ...
    void endScroll(int32_t _r)
    {
        ...
        if (hasScrollBar)
        {
            const float barY = Ibl::saturate( (float)(-(*scroll) ) / (float)sh);
...
//        BX_CHECK(_channel < 4, "Channel param must be from 0 to 3!");
    ...
    bool slider(const char* _text, float& _val, float _vmin, float _vmax, float _vinc, bool _enabled, ImguiAlign::Enum _align)
    {
        ...
        float uu = Ibl::saturate( (_val - _vmin) / (_vmax - _vmin) );
        ...
        if (isActive(id) )
        {
            ...
            if (m_dragX != m_mx)
            {
                uu = Ibl::saturate(m_dragOrig + (float)(m_mx - m_dragX) / (float)range);
    ...
    void colorWheelWidget(float _rgb[3], bool _respectIndentation, float _size, bool _enabled)
    {
        ...
        const float val = Ibl::clamped(1.0f-vv, 0.0001f, 1.0f);
        const float sat = Ibl::clamped(uu/val,  0.0001f, 1.0f);

        const float out[3] = { hsv[0], sat, val };
        Ibl::hsvToRgb(_rgb, out);
...
ImguiFontReference imguiCreate(Ibl::IDevice* _device, const void* _data, uint32_t _size, float _fontSize)
{
