...
struct vertex_index {
  size_t v_idx, vt_idx, vn_idx;
  vertex_index() {};
  vertex_index(size_t idx) : v_idx(idx), vt_idx(idx), vn_idx(idx) {};
  vertex_index(size_t vidx, size_t vtidx, size_t vnidx) : v_idx(vidx), vt_idx(vtidx), vn_idx(vnidx) {};
...
static inline size_t fixIndex(size_t idx, size_t n)
{
  size_t i;
...
static inline size_t parseInt(const char*& token)
{
  token += strspn(token, " \t");
  size_t i = atoi(token);
...
{
    const std::map<vertex_index, size_t>::iterator it = vertexCache.find(i);
  ...
  assert(in_positions.size() > (size_t)(3 * i.v_idx + 2));
  ...
  bool clearCache)
{
  ...
  for (size_t i = 0; i < faceGroup.size(); i++) {
    ...
    for (size_t k = 2; k < npolys; k++) {
      ...
      size_t v0 = updateVertex(vertexCache, shape.mesh.positions, shape.mesh.normals, shape.mesh.texcoords, in_positions, in_normals, in_texcoords, i0);
      size_t v1 = updateVertex(vertexCache, shape.mesh.positions, shape.mesh.normals, shape.mesh.texcoords, in_positions, in_normals, in_texcoords, i1);
      size_t v2 = updateVertex(vertexCache, shape.mesh.positions, shape.mesh.normals, shape.mesh.texcoords, in_positions, in_normals, in_texcoords, i2);
  ...
  std::istream& inStream)
{
  ...
  while (inStream.peek() != -1) {
    ...
    if ((0 == strncmp(token, "newmtl", 6)) && isSpace((token[6]))) {
      ...
      {
          material_map.insert(std::pair<std::string, size_t>(material.name, materials.size()));
